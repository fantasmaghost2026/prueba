...
export function Home() {
  ...
  const [finishedNovels, setFinishedNovels] = useState<any[]>([]);
  ...
  useEffect(() => {
    ...
    // Escuchar cambios en el estado del admin
    ...
    const handleAdminFullSync = (event: CustomEvent) => {
      if (event.detail.config?.novels) {
        const transmission = event.detail.config.novels.filter((novel: any) => novel.estado === 'transmision');
        const finished = event.detail.config.novels.filter((novel: any) => novel.estado === 'finalizada');
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
          ...
          <section className="mb-12">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-6">
              {novelsInTransmission.slice(0, 10).map((novel) => (
                <Link
                  to={`/novel/${novel.id}`}
                  key={`novel-transmission-${novel.id}`}
        ...
        {finishedNovels.length > 0 && (
          <section className="mb-12">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-6">
              {finishedNovels.slice(0, 10).map((novel) => (
                <Link
                  to={`/novel/${novel.id}`}
            ...
            <div className="text-center mt-8">
              ...
              <p className="text-sm text-gray-600 mt-3 max-w-md mx-auto">
                {finishedNovels.length} novelas finalizadas disponibles
              </p>
            </div>
          </section>
        )}
